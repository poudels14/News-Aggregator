...
site = {}
site[0] = "http://techcrunch.com/2014/11/06/"
...
def mine_single_site(site):
    ...
    if mineMeta:
        try:
            ...
            dotComPlace = site.find(".com")
            year = site[dotComPlace+5:26]
            month = site[dotComPlace+10:29]
            day = site[dotComPlace+13:32]
            json["year"] = int(year)
    ...
    while True:
        ...
        try:
            if (one_link["link"].find("2014") != -1):
                dotComPlace = one_link["link"].find(".com")
                year = one_link["link"][dotComPlace+5:26]
                month = one_link["link"][dotComPlace+10:29]
                if (int(year) < 2014 or int(month) < 11):
                    continue
